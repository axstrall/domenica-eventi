        ...
        <div className="w-full py-4 flex items-center justify-between">
          ...
          <Link to="/" className="flex items-center group">
            <div className="relative h-16 w-16 overflow-hidden rounded-full border-2 border-rose-200 bg-white transition-all duration-300 group-hover:scale-110">
              <img src="/LogoDomenica.png" alt="Logo Domenica Miserandino" className="h-full w-full object-cover" />
            </div>
          ...
          <div className="hidden md:flex items-center space-x-10">
            {navigation.map((link) => (
              <Link key={link.name} to={link.href} className="text-gray-600 hover:text-rose-400 font-medium uppercase tracking-widest text-sm">
                {link.name}
              </Link>
            ))}
            <a href={whatsappLink} className="bg-rose-400 text-white px-8 py-2.5 rounded-full font-bold hover:bg-rose-500 shadow-md transition-all text-xs uppercase tracking-widest">
              Richiedi Preventivo
            </a>
          </div>
