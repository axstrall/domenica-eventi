...
export function AdminPage() {
  ...
  const handleAddBrand = async (e: React.FormEvent) => {
    ...
    else alert("Errore: Il marchio potrebbe già esistere.");
  };

  const deleteBrand = async (id: string) => {
    if (confirm("Eliminare questo marchio? I prodotti collegati rimarranno senza marchio.")) {
  ...
  const saveEdit = async (id: string) => {
    ...
    const { error } = await supabase.from('products').update({ 
      name: editForm.name, brand_id: editForm.brand_id, price: isNaN(cleanPrice) ? 0 : cleanPrice 
    }).eq('id', id);
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto pt-24 space-y-12 pb-20">
      ...
      <section className="bg-white p-8 rounded-[3rem] shadow-xl border border-rose-100">
        ...
        <form onSubmit={handleAddBrand} className="flex gap-4 mb-6">
          <input type="text" placeholder="Nome Marchio (Hervit, Brandani...)" className="flex-1 border p-4 rounded-2xl outline-none" value={newBrandName} onChange={e => setNewBrandName(e.target.value)} />
          <button className="bg-rose-400 text-white px-6 rounded-2xl font-bold uppercase hover:bg-rose-500 transition-colors"><PlusCircle /></button>
        </form>
        <div className="flex flex-wrap gap-2">
          {brands.map(b => (
            <span key={b.id} className="bg-rose-50 text-rose-500 px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2 border border-rose-100">
              {b.name} <button onClick={() => deleteBrand(b.id)} className="hover:text-red-500"><X size={14}/></button>
            </span>
          ))}
        </div>
      </section>
      ...
      <section className="bg-white p-8 rounded-[3rem] shadow-xl border border-rose-100">
        ...
        <form onSubmit={handleAddProduct} className="grid gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <select className="border p-4 rounded-2xl outline-none" value={newProduct.brand_id} onChange={e => setNewProduct({...newProduct, brand_id: e.target.value})} required>
          ...
          <div className="flex items-center gap-3 bg-rose-50 p-4 rounded-2xl border border-rose-100">
            ...
            <label htmlFor="feat" className="text-rose-400 font-bold italic">Metti in evidenza nella Home Page (Vetrina)</label>
          </div>
          <button className="bg-slate-800 text-white p-5 rounded-2xl font-bold uppercase tracking-widest hover:bg-rose-500 transition-all shadow-lg">Pubblica Articolo</button>
        </form>
      </section>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Cerca..." className="pl-10 pr-4 py-2 border border-rose-100 rounded-full outline-none w-full" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
            <select className="px-4 py-2 border border-rose-100 rounded-full outline-none bg-white" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)}>
              ...
              <div className="flex items-center gap-5">
                ...
                {editingId === p.id ? (
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-rose-400 font-bold uppercase">{p.brands?.name || 'Marchio non specificato'}</p>
      ...
      {/* RUBRICA */}
      <section className="bg-white p-8 rounded-[3rem] shadow-xl border border-rose-100">
        <h2 className="text-2xl font-serif italic mb-6 flex items-center gap-2"><Database className="text-rose-400"/> Rubrica Clienti ({subscribers.length})</h2>
        <div className="space-y-3">
          {subscribers.map(s => (
            <div key={s.id} className="flex justify-between items-center p-4 bg-rose-50/30 rounded-2xl border border-rose-100">
              ...
              <a href={`https://wa.me/${s.phone.replace(/\D/g,'')}`} target="_blank" className="bg-green-500 text-white p-3 rounded-full hover:bg-green-600 transition-all shadow-md"><MessageCircle size={20}/></a>
            </div>
          ))}
        </div>
      </section>
