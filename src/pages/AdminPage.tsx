...
export function AdminPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    } catch (error) { alert("Errore foto"); }
    finally { setIsUploading(false); }
  };

  const handleAddProduct = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newProduct.image_url) { alert("Carica la foto!"); return; }
    
    try {
      const cleanPrice = parseFloat(newProduct.price.replace(',', '.'));
      const generatedSlug = (newProduct.name || 'prodotto').toLowerCase().trim().replace(/[^\w\s-]/g, '').replace(/[\s_-]+/g, '-') + '-' + Math.floor(Math.random() * 1000);
      ...
      const { error } = await supabase.from('products').insert([{
        ...
        brand: newProduct.brand, // ORA IL MARCHIO VIENE SALVATO
        price: isNaN(cleanPrice) ? 0 : cleanPrice,
        slug: generatedSlug
      }]);

      if (error) {
        alert(`Errore: ${error.message}. Assicurati che la colonna 'brand' esista su Supabase.`);
      } else {
        alert("COMPLIMENTI! ARTICOLO PUBBLICATO CON MARCHIO!");
        setNewProduct({ name: '', description: '', category_id: '', image_url: '', is_featured: true, brand: '', price: '' });
      }
    } catch (err) { alert("Errore modulo"); }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-rose-50">
        <form onSubmit={handleLogin} className="bg-white p-10 rounded-3xl shadow-xl space-y-4">
          ...
          <button className="w-full bg-rose-400 text-white p-3 rounded-full font-bold uppercase">Accedi</button>
        </form>
      </div>
    );
  }
    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto pt-24 space-y-12">
      ...
      <section className="bg-white p-8 rounded-[3rem] shadow-xl border border-rose-100">
        ...
        <form onSubmit={handleAddProduct} className="grid gap-6">
          ...
          <input type="text" placeholder="Nome Articolo (es: Vaso Fiori)" className="border p-4 rounded-2xl outline-none" value={newProduct.name} onChange={e => setNewProduct({...newProduct, name: e.target.value})} required />
          <input type="text" placeholder="Marchio (es: Hervit, Brandani...)" className="border p-4 rounded-2xl outline-none" value={newProduct.brand} onChange={e => setNewProduct({...newProduct, brand: e.target.value})} required />
          ...
          <button className="bg-slate-800 text-white p-5 rounded-2xl font-bold uppercase tracking-widest hover:bg-rose-500 transition-all">PUBBLICA ORA</button>
        </form>
      </section>
